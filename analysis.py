...
def load_accept_data(file_name):
    data = []
...
def load_reject_data(file_name):
    data = []
    with open(file_name) as f:
        column_labels = map(lambda x: x.strip("\" \n"), f.readline().split(","))
...
def generate_normalized_state_histogram(data):
    ...
    histogram_data = extract_column(data, 18)
    a, b = np.histogram(histogram_data, np.arange(50) - 0.5)
    ...
    for state in state_dict.keys():
        normalized_value = a[state_dict[state]]*1000 / float(state_populations_dict[state])
        normalized.append(normalized_value)

    py.bar(range(49), normalized, align='center')
    py.xticks(range(50))
    py.xlim([-1, 50])
    py.ylim([0, 1])
...
def main():
    col, data = load_accept_data("LoanStats3b.csv")
    ...
    # generate_histograms(data, 18, col[18], bin_counts=[np.arange(50) - 0.5])
    generate_histograms(data, 9, col[9], bin_counts=[np.arange(12) - 0.5], normalize=True)


    # reject histograms
    # -----------------
    # col, data = load_reject_data("RejectStatsA.csv")
    ...
    # print blah
    #
    # d = {a: b for a, b in zip(blah, range(len(blah)))}
    # print d

    # print len(load_reject_data("RejectStatsA.csv"))
